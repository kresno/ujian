...
class Modul_materi extends Member_Controller {
	private $kode_menu = 'modul-materi';
	private $kelompok = 'modul';
	private $url = 'manager/modul_materi';
    ...
    public function index($page=null, $id=null){
        ...
        $this->template->display_admin($this->kelompok.'/modul_materi_view', 'Mengelola Soal', $data);
    }
    ...
    function tambah(){
        ...
        $this->form_validation->set_rules('tambah-materi', 'Materi','required');
        
        if($this->form_validation->run() == TRUE){
        	$id_topik = $this->input->post('tambah-topik-id', TRUE);
        	$materi = $this->input->post('tambah-materi', FALSE);
        	$materi = str_replace(base_url(),"[base_url]", $materi);
        	$file = $_FILES['tambah-file']['name'];

        	if($id_topik=='kosong'){
        		$status['status'] = 0;
            	$status['pesan'] = 'Topik belum tersedia';
        	}else{
        		$data['materi_topik_id'] = $id_topik;
	        	$data['materi_detail'] = $soal;
	        	$data['materi_aktif'] = 1;
	        	...
	        	$this->cbt_materi_model->save($data);

	        	$status['status'] = 1;
	        	$status['pesan'] = 'Materi berhasil disimpan';
        	}
        }else{
            $status['status'] = 0;
